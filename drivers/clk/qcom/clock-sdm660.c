 ...
 * chainloaded from stock bootloader, almost every GPLL0-GPLL5 is
 * already enabled, except GPLL3.
 */
...
static const struct freq_tbl ftbl_sdcc2_apps_clk_src[] = {
	...
	F(20000000, CFG_CLK_SRC_GPLL0_EARLY_DIV, 5, 1, 3),
	F(25000000, CFG_CLK_SRC_GPLL0_EARLY_DIV, 6, 1, 2),
	F(50000000, CFG_CLK_SRC_GPLL0_EARLY_DIV, 6, 0, 0),
...
static const struct parent_map gcc_parent_map_xo_gpll0_gpll0_early_div[] = {
	{ P_XO, 0 },
	{ P_GPLL0, 1 },
	{ P_GPLL0_EARLY_DIV, 6 },  same value as GPLL0_EVEN
};
 The source selector for freq #2 (66 MHz) has the same name (GPLL0_EARLY_DIV),
 but different value (6) compared to one used above for SDCC2 clock (2).
 We can reuse another constant from clock-qcom.h with value 6 (CFG_CLK_SRC_GPLL0_EVEN)
*/
...
/*
 * With GATE_CLK_POLLED macro branch clocks have the same .en_reg and .cbcr_reg.
 * Last argument (.cbcr_reg) will be different (to 2nd one) only for vote clocks.
 */
static const struct gate_clk sdm660_clks[] = {
	GATE_CLK_POLLED(GCC_AGGRE2_USB3_AXI_CLK, GCC_AGGRE2_USB3_AXI_CBCR, BIT(0), GCC_AGGRE2_USB3_AXI_CBCR),
	GATE_CLK_POLLED(GCC_BLSP1_AHB_CLK, GCC_APCS_CLOCK_BRANCH_ENA_VOTE, BLSP1_AHB_CLK_ENA_BIT, GCC_BLSP1_AHB_CBCR),
	GATE_CLK_POLLED(GCC_BLSP1_UART2_APPS_CLK, GCC_BLSP1_UART2_APPS_CBCR, BIT(0), GCC_BLSP1_UART2_APPS_CBCR),
	GATE_CLK_POLLED(GCC_CFG_NOC_USB3_AXI_CLK, GCC_CFG_NOC_USB3_AXI_CBCR, BIT(0), GCC_CFG_NOC_USB3_AXI_CBCR),
	GATE_CLK_POLLED(GCC_SDCC1_AHB_CLK, GCC_SDCC1_AHB_CBCR, BIT(0), GCC_SDCC1_AHB_CBCR),
	GATE_CLK_POLLED(GCC_SDCC1_APPS_CLK, GCC_SDCC1_APPS_CBCR, BIT(0), GCC_SDCC1_APPS_CBCR),
	GATE_CLK_POLLED(GCC_SDCC1_ICE_CORE_CLK, GCC_SDCC1_ICE_CORE_CBCR, BIT(0), GCC_SDCC1_ICE_CORE_CBCR),
	GATE_CLK_POLLED(GCC_SDCC2_AHB_CLK, GCC_SDCC2_AHB_CBCR, BIT(0), GCC_SDCC2_AHB_CBCR),
	GATE_CLK_POLLED(GCC_SDCC2_APPS_CLK, GCC_SDCC2_APPS_CBCR, BIT(0), GCC_SDCC2_APPS_CBCR),
	GATE_CLK_POLLED(GCC_USB30_MASTER_CLK, GCC_USB30_MASTER_CBCR, BIT(0), GCC_USB30_MASTER_CBCR),
	GATE_CLK_POLLED(GCC_USB30_MOCK_UTMI_CLK, GCC_USB30_MOCK_UTMI_CBCR, BIT(0), GCC_USB30_MOCK_UTMI_CBCR),
	GATE_CLK_POLLED(GCC_USB30_SLEEP_CLK, GCC_USB30_SLEEP_CBCR, BIT(0), GCC_USB30_SLEEP_CBCR),
	GATE_CLK_POLLED(GCC_USB3_CLKREF_CLK, GCC_USB3_CLKREF_EN, BIT(0), GCC_USB3_CLKREF_EN),
	GATE_CLK_POLLED(GCC_USB3_PHY_AUX_CLK, GCC_USB3_PHY_AUX_CBCR, BIT(0), GCC_USB3_PHY_AUX_CBCR),
	GATE_CLK_POLLED(GCC_USB3_PHY_PIPE_CLK, GCC_USB3_PHY_PIPE_CBCR, BIT(0), GCC_USB3_PHY_PIPE_CBCR),
	GATE_CLK_POLLED(GCC_USB_PHY_CFG_AHB2PHY_CLK, GCC_USB_PHY_CFG_AHB2PHY_CBCR, BIT(0), GCC_USB_PHY_CFG_AHB2PHY_CBCR),
	GATE_CLK_POLLED(GCC_RX0_USB2_CLKREF_CLK, GCC_RX0_USB2_CLKREF_EN, BIT(0), GCC_RX0_USB2_CLKREF_EN),
	GATE_CLK_POLLED(GCC_RX1_USB2_CLKREF_CLK, GCC_RX1_USB2_CLKREF_EN, BIT(0), GCC_RX1_USB2_CLKREF_EN),
};
...
static ulong sdm660_gcc_set_rate(struct clk *clk, ulong rate)
{
	...
	case GCC_SDCC2_APPS_CLK:
		...
		case CFG_CLK_SRC_GPLL0_EARLY_DIV:
...
static const phys_addr_t sdm660_gpll_addrs[] = {
...
static struct msm_clk_data sdm660_gcc_data = {
	...
	/*
	 * Some data for debugging, `clk dump` command. Sadly, dump_gplls()
	 * in common clock-qcom.c code is not suitable for our PLL type.
	 * Alpha value reported for PLLs will be wrong. But on/off status - OK.
	 */
