...
static ulong sdm660_gcc_set_rate(struct clk *clk, ulong rate)
{
	...
	case GCC_SDCC2_APPS_CLK:
		...
		case CFG_CLK_SRC_GPLL4:
			...
			pr_warn("%s: cannot select GPLL4 as src for SDCC2_APPS_CLK yet, req rate: %lu", __func__, rate);
			break;
		case CFG_CLK_SRC_GPLL0_EARLY_DIV_SDCC2:
			/* enable gpll0_early */
			//clk_enable_gpll0(priv->base, &gpll0_early_clk);
			pr_warn("%s: cannot select GPLL0_EARLY as src for SDCC2_APPS_CLK yet, req rate: %lu", __func__, rate);
			break;
		}
...
static int sdm660_gcc_enable(struct clk *clk)
{
	...
	debug("%s: clk %s\n", __func__, sdm660_clks[clk->id].name);
...
static const struct qcom_reset_map sdm660_gcc_resets[] = {
	...
	//[GCC_SDCC1_BCR] = { 0x38000 }, // todo
	//[GCC_SDCC2_BCR] = { 0x1e000 }, // todo
